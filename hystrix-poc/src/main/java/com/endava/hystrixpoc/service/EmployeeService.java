...
public class EmployeeService {
	...
	public Employee getEmployeeById(int id) throws NotFoundException {
		...
		for (Employee currentEmployee : listOfEmployees) {
			if (currentEmployee.getId() == id) {
				searchedEmployee.setId(id);
				searchedEmployee.setFirstName(currentEmployee.getFirstName());
				searchedEmployee.setLastName(currentEmployee.getLastName());
				searchedEmployee.setPosition(currentEmployee.getPosition());
			}
		}
	...
	@HystrixCommand
	public Observable<Employee> getUserById(final int id) {
	  return new ObservableResult<Employee>() {
